...
\[
F=\lambda xy\overrightarrow{x}\overrightarrow{\alpha}\left[\sum_{t=x}^{t=y} f(t,\overrightarrow{x},\overrightarrow{\alpha})\right]
\]
es \(\Sigma\)-p.r.
...
Como
\[
F=G\circ[p_2^{n+2,m},p_1^{n+2,m},p_3^{n+2,m},\ldots,p_{n+m+2}^{n+2,m}],
\]
...
\(B=D_f\) es \(\Sigma\)-p.r. (Proposicion 20); por el Lema 17 tambien lo son los \(S_i,L_j\), y entonces \(C\) es \(\Sigma\)-p.r. Definimos
\[
\begin{aligned}
...
\]
Por el Lema 18, sus restricciones a \(D_1,D_2,H_1,H_2\) son \(\Sigma\)-p.r.; por el Lema 21, \(h\) y \(g\) son \(\Sigma\)-p.r.
...
\]
Luego \(G=R(h,g)\), asi que \(G\) es \(\Sigma\)-p.r. por clausura por recursion primitiva. En consecuencia \(F\) es \(\Sigma\)-p.r. por composicion.
