...
\]
Si \(F=\emptyset\), entonces \(D_F=\emptyset\) es \(\Sigma\)-p.r. Si no, por el lema de composicion, \(r=a+b\) y existen \(p,q\in\omega\) tales que
\[
...
\]
lo es. Por el Lema 19, existen extensiones totales \(\Sigma\)-p.r. \(\overline g_i:\omega^p\times\Sigma^{\ast q}\to\omega\) para \(i\le a\) y \(\overline g_i:\omega^p\times\Sigma^{\ast q}\to\Sigma^\ast\) para \(i>a\), con \(\overline g_i|_{D_{g_i}}=g_i\). Entonces, para todo \(e\in\omega^p\times\Sigma^{\ast q}\),
\[
